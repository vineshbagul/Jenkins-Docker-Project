pipeline {
        ...
        sh 'docker image tag $JOB_NAME:v1.$BUILD_ID vineshbagul/$JOB_NAME:latest'
         ...
         stage("push Image: DOCKERHUB"){
             steps{
                 ...
                 sh "docker login -u vineshbagul -p ${docker_passwd}"
                sh 'docker image push vineshbagul/$JOB_NAME:v1.$BUILD_ID'
                sh 'docker image push vineshbagul/$JOB_NAME:latest'
              ...
              sh 'docker image rm $JOB_NAME:v1.$BUILD_ID vineshbagul/$JOB_NAME:v1.$BUILD_ID vineshbagul/$JOB_NAME:latest'
              }
             }
         }
    ...
    sh "ssh -o StrictHostKeyChecking=no ubuntu@172.31.32.215 docker rmi -f vineshbagul/scripted-pipeline-demo"
    sh "ssh -o StrictHostKeyChecking=no ubuntu@172.31.32.215 docker run -p 9000:80 -d --name scripted-pipeline-demo vineshbagul/scripted-pipeline-demo:latest"
